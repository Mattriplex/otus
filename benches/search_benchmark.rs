...
use otus::{
    ...
    search::{
        ...
        minimax::{search_alpha_beta, search_minimax},
    },
};

pub fn minimax_benchmark_smart(c: &mut Criterion) {
    let mut board = Board::default();
    c.bench_function("minimax_smart", |b| {
...
pub fn minimax_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("minimax", |b| {
        b.iter(|| search_minimax(&mut board, 3, get_material_eval))
    });
}
//baseline 194ms
...
pub fn minimax_benchmark_big(c: &mut Criterion) {
    ...
    c.bench_function("minimax_big", |b| {
        b.iter(|| search_minimax(&mut board, 4, get_material_eval))
    });
}
